...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its semantic/attention plane provides versioned per-run truth, calm priority/coalescing, durable blockers, bounded local controls, metadata-only trace provenance, additive semantic JSONL, protected global accessibility settings, deterministic in-run proactivity with helpful/quiet replay evidence, and a canonical bounded Experiential Layer whose reviewed advice cannot override runtime truth; append-only screen-reader presentation, human AT validation, and opt-in watchers/voice follow.
