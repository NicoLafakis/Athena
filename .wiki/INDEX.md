...
- [Permissions and project trust](architecture/permissions-trust.md) - `permissionMode` vs. the per-machine trust registry, the one-way trust bootstrap (explicitly trusted projects start in trusted mode; win32 lifts the sandbox to unrestricted because no backend exists), and which rails (deny rules, resource policy, hooks) never relax.
