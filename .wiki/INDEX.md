...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its passive semantic and attention plane provides versioned per-run state, runtime-over-agent truth, calm priority/coalescing, durable blockers, bounded local status/detail, and metadata-only trace provenance; append-only screen-reader presentation, experiential guidance, governed proactivity, and opt-in watchers/voice follow in the ordered plan.
