...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its shared semantic plane feeds deterministic attention, bounded experience guidance, a real-process-tested append-only screen-reader composition with cancellation/resume and command parity, resource-scoped watcher primitives with a real backend probe, and a provider-neutral voice core with current Realtime model/wire research; secure non-chattery first-run auth, live audio/wake-word probes, and human AT/cross-platform validation remain.
