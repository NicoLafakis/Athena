...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its semantic/attention plane provides versioned per-run truth, calm priority/coalescing, durable blockers, bounded local controls, protected accessibility settings, deterministic proactivity, bounded experience guidance, a byte-tested append-only presentation core, and an explicit resource-scoped foreground watcher foundation with a real backend probe; visible composition, human AT/cross-platform validation, and opt-in voice follow.
