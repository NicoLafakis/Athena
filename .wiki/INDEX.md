...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its semantic/attention plane provides versioned per-run truth, calm priority/coalescing, durable blockers, bounded local controls, metadata-only trace provenance, and protected global accessibility settings with nonfatal recovery; append-only screen-reader presentation, experiential guidance, governed proactivity, and opt-in watchers/voice follow in the ordered plan.
