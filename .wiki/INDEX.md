...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its shared semantic plane feeds deterministic attention, bounded experience guidance, a real-process-tested append-only screen-reader composition with silent auth and command parity, resource-scoped watcher primitives with a real backend probe, and an opt-in Windows Athena-wake/Realtime voice conductor with confirmed durable engine delegation; live audio/account probes and human AT/cross-platform validation remain.
