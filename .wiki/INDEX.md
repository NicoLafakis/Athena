...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its shared semantic plane feeds deterministic attention, bounded experience guidance, a real-process-tested append-only screen-reader composition with local controls and accessible permissions, resource-scoped watcher primitives with a real backend probe, and a provider-neutral voice core; secure non-chattery first-run auth, current Realtime/audio probes, and human AT/cross-platform validation remain.
