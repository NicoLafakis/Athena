...
- [Blind-first Jarvis upgrade](features/blind-first-jarvis/00-overview.md) - Canonical Tier 3 package, now in implementation. Its shared semantic plane feeds deterministic attention, bounded experience guidance, a byte-tested append-only presentation core, resource-scoped watcher primitives with a real backend probe, and a provider-neutral voice core with deterministic routing, speech ownership, and local confirmation; visible composition, current Realtime/audio probes, and human AT/cross-platform validation remain.
