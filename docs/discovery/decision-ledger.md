...
| D-015 | What failure should the draft prioritize before Nico answers the pre-mortem? | assumed-default | Guard equally against excessive interruption, false confidence, and an accessible mode that cannot complete real work. | agent-default: these are the highest-impact failures found by internal pre-mortem. | Provides a temporary risk priority without inventing founder evidence. | Nico answers Q-003. |
| D-016 | Is there already a voice architecture to reuse? | superseded | The earlier answer preserved the opt-in daemon, wake word, Realtime conductor, session router, permissions, and phased control channel. Its transport and safety seams remain reusable, but D-022 supersedes the conductor as the product boundary. | reconciled artifact: `.wiki/features/blind-first-jarvis/voice.md`; historical commit `3bc1ce6`; live voice findings (2026-07-29). | Preserves the provenance of the intermediate implementation without presenting it as the target architecture. | D-022 is superseded. |
| D-017 | Which Realtime model is pinned in the implementation? | answered-by-spike | Use `gpt-realtime-2.1-mini` as the lower-cost default and allow `gpt-realtime-2.1` as the quality option. Recheck availability, API contracts, and prices before support or cost claims. | official OpenAI documentation and working Realtime probe checked 2026-07-29. | Replaces the stale draft `gpt-realtime-2` name with the models actually configured and tested. | OpenAI model availability or API contracts change. |
...
| D-020 | What may the first foreground watcher observe? | default-applied | One explicitly named existing file or direct directory inside the current resource-policy workspace. V1 is non-recursive, notification-only, and stops on Ctrl-C/process exit; missed events never authorize, block, or verify work. | R-009, threat model, and foreground-first D-009. | Bounds the watcher implementation and its safety consequences. | Dogfood establishes a concrete recursive, external-resource, or persistent-service need. |
| D-021 | Can the Realtime voice model authorize a consequential action? | answered-by-safety-invariant | No. In the intermediate conductor, a separate exact recognized `confirm` tied to the opaque pending ID is required. In direct-harness voice, only Athena's canonical permission record and resolver may authorize the action; Realtime supplies untrusted turn/control input. | D-010, R-005, voice threat review, and ADR 0003 (2026-07-29). | Prevents model-authored speech or tool arguments from bypassing human control. | Never; recognition UX may change but Realtime cannot authorize work. |
| D-022 | What is the target voice architecture after the audio proof? | answered | `athena voice` owns one Athena harness session directly. A persistent local wake listener releases post-wake PCM to Realtime, which only interprets intent and speaks Athena's authoritative result. The conductor/delegate proxy and per-task CLI child are removed; active-TUI IPC remains deferred. | `.wiki/features/blind-first-jarvis/direct-harness-voice.md`; ADR 0003; Nico's direct-harness clarification (2026-07-29). | Makes voice a true hands-free surface over Athena instead of a second assistant in front of it. | Nico explicitly selects a different product boundary. |

## Priority-ranked open questions

| ID | Question (plain language) | Status | Decision | Source | Why it matters / unblocks | Revisit trigger |
|---|---|---|---|---|---|---|
| Q-001 | Which milestone should lead: the semantic/screen-reader foundation or the voice capability spike? | default-applied | Ship the semantic truth plane before conversational status and voice permissions; the audio/probe spike remains independently schedulable. | D-019 | The direct voice adapter needs trustworthy harness state and announcements; sequencing determines the critical path. | Nico explicitly selects a different order. |
...
| 10. Integrations | Clear | Existing systems, the working Realtime/wake proof, and the direct-harness target boundary are defined; model choice was resolved during the voice spike. |
...
- Live testing proved the raw microphone-to-Realtime-to-Marin path but also exposed that
  the conductor/delegate composition is a proxy rather than direct Athena conversation.
  D-016 is therefore superseded by D-022 and the direct-harness specification/ADR.
