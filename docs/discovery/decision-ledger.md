...
| D-011 | Where does relevant past experience come from? | inferred | The existing Experiential Layer plan; this project does not create another retrieval or memory store. | artifact: `docs/Athena Experiential Layer v1 — Implementation Plan.pdf`. | Prevents duplicate memory authority and implementation. | The experiential plan is rejected or redesigned. |
...
| D-016 | Is there already a voice architecture to reuse? | inferred | Yes. The tracked remote commit defines an opt-in `athena voice` daemon, wake word, OpenAI Realtime conductor, engine-session router, permissions, and phased control channel. This upgrade supplies its trustworthy semantic digest rather than forking voice. | artifact: commit `3bc1ce6`, `docs/voice/athena-voice-mode-plan-2026-07-28.md`. | That voice plan is superseded. |
...
| 10. Integrations | Clear | Existing systems plus tracked Realtime/wake-word/session-routing plan are defined; model choice is resolved at the voice spike. |
...
- The tracked remote voice plan was discovered after the first draft. D-008/D-009 were
