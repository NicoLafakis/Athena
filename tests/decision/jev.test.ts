...
describe('Jev recall router', () => {
  it('sends only the redacted current request and gets typed recall and memory-intake decisions in one call', async () => {
    ...
    expect(routeQuestion).toContain('show the conversation about an identifiable subject')
    expect(routeQuestion).toContain('name alone does not make it a decision')
